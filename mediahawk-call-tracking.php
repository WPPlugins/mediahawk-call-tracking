...
if ( !class_exists( 'mediahawk-call-tracking' ) ) {
	...
	class mediahawk_call_tracking {
		...
		function admin_init() {
			...
			register_setting( 'insert-mediahawk-details', 'optimizely', 'trim' );
			register_setting( 'insert-mediahawk-details', 'hubspot', 'trim' );
		}
		function admin_menu() {
		...
		function wp_footer() {
			if ( !is_admin() && !is_robots() && !is_trackback() && !is_feed() ) {
				...
				if($active == 1){
					...
					//Integrations
					$optimizely = get_option( 'optimizely', '' );
					$hubspot 	= get_option( 'hubspot', '' );
					if($optimizely == 1){
						$opt = 'var optimizelyActive = true;';
					}
					if($hubspot == 1){
						$hub = 'var hubspotActive = true;';
					}
...
".$opt."
".$hub."
document.write(unescape(\"%3Cscript src='\"+mhProtocol+\"www.dynamicnumbers.mediahawk.co.uk/getVisitorParams.js?mhRand=\"+mhRand+\"' type='text/javascript'%3E%3C/script%3E\"));</script>
<script type=\"text/javascript\">
var mhAccountRef = '".$tracking_id."';
document.write(unescape(\"%3Cscript src='\"+mhProtocol+\"www.dynamicnumbers.mediahawk.co.uk/getVisitorNumber.php?mhAccountRef=\"+mhAccountRef+mhUrlParamString+\"&mhRand=\"+mhRand+\"' type='text/javascript'%3E%3C\/script%3E\"));
</script> 
<script type=\"text/javascript\">
try {
mhChangeTelNumber();
...
<script type=\"text/javascript\"> 
try {
mhChangeTelNumberText(\"textNumber1\");
}
catch (err){}
</script>
";
				...
				<?php screen_icon(); ?>
					...
					<div class="mediahawk-wrap" style="width: 360px;float: left;margin-right: 2rem;">
						...
						<form name="dofollow" action="options.php" method="post">
							...
							<hr/>
							<h3 class="mediahawk-labels" for="Integrations">Integrations</h3>
							<h4 class="mediahawk-labels" for="Integrations">Optimizely</h4>
							<input type="radio" name="optimizely" value="1" <?php if(esc_html( get_option( 'optimizely' ) ) == 1){echo 'checked';}  ?> />On<br />
                        	<input type="radio" name="optimizely" value="0" <?php if(esc_html( get_option( 'optimizely' ) ) == 0){echo 'checked';}  ?> />Off<br />
                        	<h4 class="mediahawk-labels" for="Integrations">Hubspot</h4>
							<input type="radio" name="hubspot" value="1" <?php if(esc_html( get_option( 'hubspot' ) ) == 1){echo 'checked';}  ?> />On<br />
                        	<input type="radio" name="hubspot" value="0" <?php if(esc_html( get_option( 'hubspot' ) ) == 0){echo 'checked';}  ?> />Off<br />
						<p class="submit">
